...
const char HTTP_SCRIPT[] PROGMEM          = "<script> function httpget(url) { const Http = new XMLHttpRequest(); Http.open(\"GET\", url); Http.send(); } function c(l){document.getElementById('s').value=l.innerText||l.textContent;document.getElementById('p').focus();}</script>";
