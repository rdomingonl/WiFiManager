...
const char HTTP_SCRIPT[] PROGMEM          = "<script> function httpget(url) { alert('httpget ' + url); const Http = new XMLHttpRequest(); Http.open(\"GET\", url); Http.send(); } function c(l){document.getElementById('s').value=l.innerText||l.textContent;document.getElementById('p').focus();}</script>";
const char HTTP_HEAD_END[] PROGMEM        = "</head><body><div style='text-align:left;display:inline-block;min-width:260px;'>";
const char HTTP_PORTAL_OPTIONS[] PROGMEM  = "<form action=\"/wifi\" method=\"get\"><button>Configure WiFi</button></form><br/><form action=\"/0wifi\" method=\"get\"><button>Configure WiFi (No Scan)</button></form><br/><form action=\"/i\" method=\"get\"><button>Info</button></form><br/><form><button onclick=\"javascript:httpget('/e');\">Exit</button></form><br/><form action=\"/r\" method=\"post\"><button>Reset</button></form>";
...
class WiFiManager
{
  public:
    ...
    boolean       autoConnect(char const *apName, char const *apPassword = NULL, boolean startConfigPortalOnConnectionFailure = true);
